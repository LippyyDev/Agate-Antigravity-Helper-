  ...
  static async poll() {
    ...
    try {
      ...
      for (const account of accounts) {
        // Skip accounts that need re-login — their token is irrecoverable until the user re-authenticates
        if (account.status === 'decryption_error') {
          logger.warn(`Monitor: Skipping account ${account.email} (decryption_error — re-login required)`);
          continue;
        }
