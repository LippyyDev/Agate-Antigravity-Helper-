...
export class TokenManagerService implements OnModuleInit {
  ...
  private async finalizeSelectedToken(
    ...
    try {
      ...
      if (nowSeconds >= tokenData.expiry_timestamp - 300) {
        // Distinguish: token already past its expiry vs merely approaching it.
        const isAlreadyExpired = nowSeconds >= tokenData.expiry_timestamp;
        ...
        try {
          ...
          if (isAlreadyExpired) {
            // Token is definitely expired and refresh failed — propagate so this
            // account is skipped by the outer retry loop rather than sending an
            // expired token to upstream (which would trigger a 401 + 30-min cooldown).
            throw e instanceof Error ? e : new Error(String(e));
          }
          // Token is near expiry but may still be valid — proceed with existing token.
          this.logger.warn(`Proceeding with near-expiry token for ${tokenData.email}`);
        }
      }
   ...
   * Called reactively by proxy.service when upstream returns a 401 Unauthorized,
   * so we attempt a refresh before falling back to account rotation or cooldown.
   * Returns true on success, false on failure.
   */
  async forceRefreshToken(accountId: string): Promise<boolean> {
    const tokenData = this.tokens.get(accountId);
    if (!tokenData) {
      this.logger.warn(`forceRefreshToken: account ${accountId} not found in pool`);
      return false;
    }

    this.logger.log(`Force-refreshing token for ${tokenData.email} after upstream 401...`);
    try {
      ...
      this.logger.log(`Force-refresh successful for ${tokenData.email}`);
      return true;
    } catch (e) {
      this.logger.error(`Force-refresh failed for ${tokenData.email}`, e);
      return false;
    }
  }
