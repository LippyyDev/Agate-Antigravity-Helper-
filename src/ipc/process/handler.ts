  ...
  try {
    ...
    for (const proc of processes) {
      ...
      if (
        ...
        cmd.includes('antigravity-helper')
      ) {
        continue;
      }
...
export async function closeAntigravity(): Promise<void> {
  ...
  try {
    ...
    const targetProcessList = getProcesses().filter((p) => {
      ...
      // Exclude this electron app (if named Antigravity Helper or antigravity-helper)
      if (p.cmd.includes('Antigravity Helper') || p.cmd.includes('antigravity-helper')) {
        return false;
      }
