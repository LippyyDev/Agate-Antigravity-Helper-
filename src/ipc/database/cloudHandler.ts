...
import { accounts, categories, itemTable, settings } from './schema';
...
function ensureDatabaseInitialized(dbPath: string): void {
  ...
  try {
    ...
    // Migration: Check columns on accounts table
    ...
    if (!hasCategoryId) {
      db.exec('ALTER TABLE accounts ADD COLUMN category_id TEXT');
    }
...
export class CloudAccountRepo {
  ...
  static async getAccounts(): Promise<CloudAccount[]> {
    ...
    try {
      ...
      const DECRYPTION_ERROR_CODE = 'ERR_DATA_MIGRATION_FAILED';

      /** Sentinel token used only for accounts in 'decryption_error' state — never used for API calls. */
      const EMPTY_TOKEN_SENTINEL: import('../../types/cloudAccount').CloudTokenData = {
        access_token: '',
        refresh_token: '',
        expires_in: 0,
        expiry_timestamp: 0,
      ...
      for (const normalizedRow of rows) {
        ...
        try {
          ...
          // If the master key has rotated and the old ciphertext is irrecoverable,
          // surface the account as 'decryption_error' so the UI can prompt re-login
          // instead of crashing the entire list request.
          const errorMessage = error instanceof Error ? error.message : String(error);
          if (errorMessage.includes(DECRYPTION_ERROR_CODE)) {
            cloudAccounts.push({
              id: normalizedRow.id,
        ...
        try {
          ...
          // Quota is non-critical — proceed without it rather than aborting the account
        ...
        cloudAccounts.push({
          ...
          name: normalizedRow.name ?? undefined,
          label: normalizedRow.label ?? undefined,
          category_id: normalizedRow.categoryId ?? undefined,
          avatar_url: normalizedRow.avatarUrl ?? undefined,
  ...
  static async updateAccountCategory(id: string, categoryId: string | null): Promise<void> {
    const dbPath = getCloudAccountsDbPath();
    ensureDatabaseInitialized(dbPath);
    const db = new Database(dbPath);
    try {
      configureDatabase(db, { busyTimeoutMs: SQLITE_BUSY_TIMEOUT_MS });
      db.prepare('UPDATE accounts SET category_id = ? WHERE id = ?').run(categoryId, id);
    } finally {
      db.close();
    }
  }
  ...
  static getCategories(): Category[] {
    ...
    try {
      configureDatabase(db, { busyTimeoutMs: SQLITE_BUSY_TIMEOUT_MS });
      const rows = db.prepare('SELECT id, name, color, created_at FROM categories ORDER BY created_at ASC').all() as {
        id: string;
        name: string;
        color: string;
        created_at: number;
  ...
  static createCategory(id: string, name: string, color: string): Category {
    const dbPath = getCloudAccountsDbPath();
    ensureDatabaseInitialized(dbPath);
    const db = new Database(dbPath);
    ...
    try {
      configureDatabase(db, { busyTimeoutMs: SQLITE_BUSY_TIMEOUT_MS });
      db.prepare('INSERT INTO categories (id, name, color, created_at) VALUES (?, ?, ?, ?)').run(id, name, color, now);
      logger.info(`Created category: ${name} (${id})`);
  ...
  static updateCategory(id: string, name: string, color: string): void {
    const dbPath = getCloudAccountsDbPath();
    ensureDatabaseInitialized(dbPath);
    const db = new Database(dbPath);
    try {
  ...
  static deleteCategory(id: string): void {
    ...
    try {
      configureDatabase(db, { busyTimeoutMs: SQLITE_BUSY_TIMEOUT_MS });
      // Clear category_id from accounts that were using this category
      db.prepare('UPDATE accounts SET category_id = NULL WHERE category_id = ?').run(id);
  ...
  static async updateLabel(id: string, label: string): Promise<void> {
    const { raw, orm } = getCloudDb();
    try {
      orm.update(accounts).set({ label }).where(eq(accounts.id, id)).run();
    } finally {
      raw.close();
    }
  }
